<article class="compartilhe-monteiro">
	<div class="tamanho-max">
		
		<div class="col-xs-12 col-sm-3 col-md-3 pd-left-5 center-bloco-compartilhe">
			<h2 class="titulo-compartilhe font-normal">Assine nossa Newsletter</h2>
			<form class="form-search" method="post" name="newsletter">
	            <input class="font-normal form-compartilhe" type="text" name="newsletter" placeholder="e-mail">
	            <input class="btn-ok cinza font-normal" type="submit" value="ok">
	         </form>
		</div>
		<div class="col-xs-12 col-sm-1 col-md-1">&nbsp;</div>
		<div class="col-xs-12 col-sm-3 col-md-3  center-bloco-compartilhe">
			<h2 class="titulo-compartilhe font-normal">Indique este site</h2>
			<form class="form-search" method="post" name="indique">
	            <input class="font-normal form-compartilhe" type="text" name="indique" placeholder="e-mail">
	            <input class="btn-ok cinza font-normal" type="submit" value="ok">
	         </form>
		</div>
		<div class="col-xs-12 col-sm-1 col-md-1">&nbsp;</div>
		<div class="col-xs-12 col-sm-4 col-md-4 pd-right-5 center-bloco-compartilhe">
			<h2 class="titulo-compartilhe font-normal">Fique conectado conosco</h2>
				<div class="bloco-conectado">
					<a href="" target="_blank">
						<img src="img/face.png" alt"Facebook" title="Facebook">		
					</a>
					<a href="" target="_blank">
						<img src="img/likedin.png" alt"Likedin" title="Likedin">		
					</a>
					<a href="" target="_blank">
						<img src="img/youtube.png" alt"Youtube" title="Youtube">		
					</a>
				</div>
		</div>
		
	</div>
</article>
<section class="encapsula-rodape">
	<footer>
		<div class="row">
			<div class="col-xs-12 col-sm-3 col-md-3 hidden-xs bloco-sobre-footer">
				<a href="index.php">
					<img src="img/logo-footer.png" alt="Marcos Monteiro Sociedade de Advogados" title="Marcos Monteiro Sociedade de Advogados">
				</a>
				<p class="txt-logo-footer font-normal">Sociedade de profissionais altamente qualificados, aptos a prestar assessoria, consultoria e direção jurídica de modo diferenciado, padronizado ou personalizado, de acordo com as exigências e o dinamismo do mercado. </p>
			</div>
			<div class="col-xs-12 col-sm-1 col-md-1">&nbsp;</div>
			<div class="col-xs-12 col-sm-5 col-md-3">
				<div class="bloco-center-footer">
					<p class="txt-titulo-footer font-normal">Localização</p>

					<div class="linha-local-footer">
						<img class="icon-local-footer" src="img/icon-casa.png" alt="icone casa" title="icone casa">
						<p class="txt-local-footer font-normal">Rua Maria Paula, 36 | cj. A - 3º andar <br>CEP: 01319-000 | Bela Vista | SP</p>
					</div>
					<div class="linha-local-footer">
						<img class="icon-local-footer" src="img/icon-cel.png" alt="icone telefone" title="icone telefone">
						<p><a class="txt-local-footer font-normal" href="tel:01139298934">55 11 3929-8934</a>  |  <a class="txt-local-footer font-normal" href="tel:01139298936">Fax: 55 11 3929-8936</a></p>
						<img class="icon-local-footer top-linha-footer" src="img/icon-waths.png" alt="icone wathsapp" title="icone wathsapp">
						<p class="top-linha-footer"><a class="txt-local-footer font-normal" href="tel:011987492993">55 11 9 8749-2993</a></p>
					</div>
					<div class="linha-local-footer">
						<img class="icon-local-footer" src="img/icon-msg.png" alt="icone mensagem" title="icone mensagem">
						<a class="txt-local-footer font-normal" href="mailto:contato@mmonteiroadv.com.br">contato@mmonteiroadv.com.br</a>
					</div>
				</div>
			</div>
			<div class="col-xs-12 col-sm-1 col-md-1">&nbsp;</div>
			<div class="col-xs-12 col-sm-5 col-md-4">
				<div class="bloco-center-footer">
					<p class="txt-titulo-footer font-normal">Curta nossa página!</p><br>
					<div class="fb-page" data-href="https://www.facebook.com/Marcos-Monteiro-Sociedade-de-Advogados-798300076864104/timeline/" data-width="280" data-height="250" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true" data-show-posts="false"></div>
				</div>
			</div>
		</div>
		<div class="row bloco-direitos">
			<div class="col-xs-12 col-sm-12 col-md-12 encapsula-direitos">
				<p class="txt-direitos font-normal">MARCOS MONTEIRO SOCIEDADE DE ADVOGADOS - Todos os direitos reservados - 2015</p>
				<a class="fl-right" href="http://www.planobweb.com.br/"><img src="img/logo-planob.png" alt="PlanoBWeb" title="PlanoBWeb"></a>
				<p class="txt-dev font-normal">Desenvolvimento:</p>
			</div>
		</div>
	</footer>
</section>
<!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
<!-- Include all compiled plugins (below), or include individual files as needed -->
<script src="js/bootstrap.min.js"></script>
<!-- vitrine -->
<script src="js/vitrine/owl.carousel.js"></script>
<!-- vitrine -->
<script src="js/geral.js" type="text/javascript"></script>
<script>
$(document).ready(function() {
 $("#owl-demo").owlCarousel({
    navigation : true,
    slideSpeed : 300,
    paginationSpeed : 400,
    singleItem : true
 });
});
</script>